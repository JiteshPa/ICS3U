...
public class AdventureGame {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		try {
			...
			theme.open(AudioSystem
					.getAudioInputStream(new File("Stephen_Walking_It_Came_From_Planet_Earth_livelagu.wav"))); // open
																												// the
																												// given
																												// file
																												// for
			...
			theme.start();
			// Instead of using start you could alternatively use the loop
			// function.
		...
		while (playAgain == true) {
			...
			if (userInput[0][0].equalsIgnoreCase("forest")) {
				// tree
				log.println("The guards chase you into the forest!");
				...
				if (userInput[0][1].equalsIgnoreCase("hide in tree")) {
					...
					if (userInput[0][2].equalsIgnoreCase("Yes")) {
						...
						if (chance == 0 || chance == 1) {
							// body
							log.println("You miss, the guards spot and kill you. You are dead.");
							death = true;

						} else {
							// dead guards
							log.println(
									"You hit the guards and knock them out. You climb down and steal some of their gold, along with a sword as a weapon");
							sword = true;
					...
					if (userInput[0][3].equalsIgnoreCase("the bridge") && death == false) {
						...
						if (triesTroll == 0) {
							log.println(
									// dead troll on bridge with guard standing
									// over it
